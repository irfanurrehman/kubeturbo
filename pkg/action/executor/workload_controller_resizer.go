...
func (r *WorkloadControllerResizer) Execute(input *TurboActionExecutorInput) (*TurboActionExecutorOutput, error) {
	...
	// Atleast 1 pod is needed because it contains the node info, which
	// subsequently is needed to get the cpufrequency.
	// TODO(irfanurrehman): This can be slightly erratic as the value conversion will always
	// use the node frequency of the first pod.
